...
public class SHAPanel extends JPanel{
	...
	public SHAPanel() {
		...
		JLabel lblTitle = new JLabel("SHA");
		...
		btnHash.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String type = typeSHA.getSelectedItem().toString();
				...
				if (!txtInput.getText().equals("") && fileChooser1 == null) {
					String hashCode = sha.convertStringToHash(txtInput.getText(),type);
				...
				if (txtInput.getText().equals("") && fileChooser1 != null) {
					File selectedFile = fileChooser1.getSelectedFile();
					System.out.println("Selected file: " + selectedFile.getAbsolutePath());
					String hashCode = sha.convertFileToHash(selectedFile.getAbsolutePath(),type);
					txtOutput.setText(hashCode);
		...
		add(btnCheck);
		 ...
		 typeSHA = new JComboBox();
		typeSHA.setFont(new Font("Tahoma", Font.PLAIN, 12));
		typeSHA.setModel(new DefaultComboBoxModel(new String[] {"SHA-1", "SHA-224", "SHA-256", "SHA-384", "SHA-512"}));
		typeSHA.setBounds(289, 52, 100, 21);
		add(typeSHA);
		...
		btnCheck.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (fileChooser2 != null && !txtHashCode.getText().equals("")) {
					String type = typeSHA.getSelectedItem().toString();
					File selectedFile = fileChooser1.getSelectedFile();
					boolean isNotChange= sha.checkFile(selectedFile.getAbsolutePath(), txtHashCode.getText(),type);
					if(isNotChange) {
