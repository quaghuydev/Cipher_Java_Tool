...
import model.Camellia;

public class CamelliaPanel extends JPanel {
	...
	public boolean isValidCamelliaKey(String key, int limitKey) {
		try {
	...
	public CamelliaPanel() {
		...
		setLayout(null);// TODO Auto-generated constructor stub
	//length key
...
//title
		JLabel lbTitle = new JLabel("CAMELLIA");
		lbTitle.setHorizontalAlignment(SwingConstants.CENTER);
		lbTitle.setFont(new Font("Tahoma", Font.BOLD, 20));
		lbTitle.setBounds(200, 21, 150, 30);
		add(lbTitle);
//create key
		...
		btnCreateKey.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					key = camellia.createKey(Integer.parseInt(valueKey));
				} catch (Exception e1) {
				...
				if (key != null) {
					...
					if (fileExport != null && fileChooser != null) {
						try {
							...
							camellia.encryptFile(fileChooser.getSelectedFile().getAbsolutePath(),
									creatPath("Camellia_encrypt", fileChooser, fileExport), key);
					...
					if (!txtInput.getText().equals("")) {
						try {
							txtOutput.setText(camellia.encryptToBase64(key, txtInput.getText()));
							System.out.println(camellia.encryptToBase64(key, txtInput.getText()));
				...
				if (key != null) {
					...
					if (fileExport != null && fileChooser != null) {
						try {
							...
							camellia.decryptFile(fileChooser.getSelectedFile().getAbsolutePath(),
									creatPath("Camellia_decrypt", fileChooser, fileExport), key);
					...
					if (!txtInput.getText().equals("")) {
						try {
							txtOutput.setText(camellia.decryptFromBase64(key, txtInput.getText()));
							System.out.println(camellia.decryptFromBase64(key, txtInput.getText()));
						} catch (Exception e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
